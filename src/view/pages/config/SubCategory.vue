<template>
  <div class="d-flex flex-row">
    <b-modal id="sub-category-form" @hide="reset" centered>
      <template #modal-header="{ close }">
        <h5>{{ (!form.id ? "New" : "Update") + " Sub-category" }}</h5>
        <b-button
          type="button"
          aria-label="Close"
          class="close"
      ...
      <alert-success :form="form"
        >Sub-category saved successfully!</alert-success
      >
      ...
      >
        <b-form-group label="Category">
          <b-form-select v-model="form.category_id" :options="categories">
            <template #first>
    ...
    <div class="flex-row-fluid">
      <div class="card card-custom">
        ...
        <div class="card-body">
          ...
          <b-pagination-nav
            v-if="subCategoryList && subCategoryList.data"
            :link-gen="linkGen"
            :number-of-pages="subCategoryList.last_page"
            first-text="First"
            prev-text="Prev"
            next-text="Next"
            last-text="Last"
...
import {
  ...
  DESTROY,
  RESTORE
} from "@/core/services/store/sub-category.module";
...
export default {
  ...
  data() {
    return {
      ...
      categories: [],
      query: "filter[trashed]=with&include=apartmentsCount,category"
    };
  },
  methods: {
    fetchAll() {
    ...
    edit(subCategory) {
      ...
      this.current_icon = subCategory.icon;
      this.$bvModal.show("sub-category-form");
    },
    update() {
      this.$store.dispatch(UPDATE, this.form).then(() => {
        this.reset();
        this.fetchAll();
        this.$bvModal.hide("sub-category-form");
      });
    },
  ...
  computed: {
    ...
    subCategories() {
      return this.subCategoryList.data;
    }
  },
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{ title: "Sub-categories" }]);
