<template>
  <div class="d-flex flex-row">
    <b-modal
      id="termination-form"
      v-if="selectedApartmentBooking"
      @hide="reset()"
      centered
    >
      <template #modal-header="{ close }">
        <h5>
          Terminate {{ selectedApartmentBooking.user.full_name }}'s Rental for
          {{ selectedApartmentBooking.apartment.name }}
        </h5>
        <b-button
          type="button"
          aria-label="Close"
    ...
    <div class="flex-row-fluid">
      <b-card>
        ...
        <b-pagination-nav
          v-if="bookingsList && bookingsList.data"
          :link-gen="linkGen"
          :number-of-pages="bookingsList.last_page"
          first-text="First"
          prev-text="Prev"
          next-text="Next"
          last-text="Last"
...
import { FETCH_ALL } from "@/core/services/store/apartmentbooking.module";
import { TERMINATE } from "@/core/services/store/apartmentrental.module";
...
export default {
  ...
  methods: {
    ...
    openTerminationForm(apartmentRental) {
      ...
      this.selectedApartmentBooking = apartmentRental;
  ...
  computed: {
    ...mapGetters({
      bookingsList: "getApartmentBookings"
    }),
    rentals() {
      return this.bookingsList.data;
    }
  },
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{ title: "Apartment Bookings" }]);
  }
};
</script>
